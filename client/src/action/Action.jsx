...
export function getPokes(){
    return async (dispatch) => {
        var pokes = await axios.get('http://localhost:5173/pokemons')
        return dispatch({
            type: "GET_POKEMONS",
            payload: pokes.data
        })
...
export function getTypes(){
    return async (dispatch) => {
        var types = await axios.get('http://localhost:5173/types')
        return dispatch({
            type: "GET_TYPES",
            payload: types.data
        })
...
export function getDetails(id) {
    return async function (dispatch) {
        try {
            let poke = await axios.get(`http://localhost:5173/pokemons/${id}`)
            return dispatch({
                type: 'GET_DETAILS',
                payload: poke.data
            })
...
export function postPoke(payload) {
    return async function (dispatch) {
        const pokemonCreated = await axios.post("http://localhost:5173/pokemons", payload)
